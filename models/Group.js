...
Group.post('save', async function (group) {
  ...
  await Space.updateMany({ space_id: { $in: spaceIds } }, { $addToSet: { space_groups_id: groupId } });
  ...
  const spaceGroupIds = await Space.distinct('space_groups_id', { space_id: { $in: spaceIds } });
  const users = await User.find({ user_groups: { $in: spaceGroupIds } });

  // обновляем поле user_spaces у найденных Пользователей
  const userUpdates = users.map(user => User.updateOne({ _id: user._id }, { $addToSet: { user_spaces: { $each: spaceIds } } }));
